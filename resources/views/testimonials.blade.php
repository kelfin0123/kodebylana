...
<div class="max-w-5xl mx-auto px-4 py-14">
  <header class="text-center">
    ...
    <p class="mt-2 text-slate-600 dark:text-slate-300">Apa kata klien tentang KodeByLana.</p>
  </header>
    ...
    <div class="mt-10 grid gap-6 sm:grid-cols-2 lg:grid-cols-3">
      @foreach($testimonials as $t)
        @php
          ...
          $rating = isset($t->rating) && is_numeric($t->rating) ? max(0, min(5, (int)$t->rating)) : null;
        @endphp
          ...
          <div class="flex items-center gap-4">
            ...
            <div class="min-w-0">
              ...
              <div class="text-xs text-slate-500 flex items-center gap-1">
                @if(!empty($t->role)) <span class="truncate">{{ $t->role }}</span> @endif
                @if(!empty($t->company))
                  <span>&middot;</span>
                  @if(!empty($t->link))
          ...
          @if(!empty($t->project_title) || !empty($t->created_at))
            <div class="mt-4 flex items-center justify-between text-xs text-slate-500">
              ...
              @if(!empty($t->created_at))
                <time datetime="{{ $t->created_at->toDateString() }}">{{ $t->created_at->format('M Y') }}</time>
              @endif
            </div>
          @endif
        </article>
